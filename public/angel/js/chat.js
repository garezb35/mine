...
function connect(type = "public")
{
    try{
        ...
        if(socket == null)
        {
            socket = io.connect('http://'+server_domain+':7443'+'/'+type,socketOption);
        }
        sendProcess('login');
    }
    catch(e){
...
function chatManager(type,nick)
{
    ...
    {
        windowOpen('/box_chatting','chatPopup',330,450,'no');
    }
    else if(type == 'help')
    {
...
function printChatMsg(level,sex,mark,useridKey,nickname,msg,item,winFixCnt,image)
{
    ...
    else
    {
        ...
        $('#msgBox').append('<li'+addClass+'>'+markView+'<span style="position:relative;"><img src="/angel/img/level/'+image+'" width="30" height="30" orgLevel="'+sex+level+'"'+gasmaskClass+'/>'+itemView+winFixCntView+'</span> <strong><a href="#" onclick="return false;" title="'+nickname+'" rel="'+useridKey+'" class="uname">'+familyNickView+nickname+'</a></strong> '+msg+'</li>');
    }
        ...
        list.forEach(element => {
            if(element.id.includes('null-'))
            {
                html += '<li id="'+element.id+'">\
                            <img src="/angel/img/level/'+element.image+'" width="30" height="30">\
