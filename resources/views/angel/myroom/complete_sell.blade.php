...
<div @class('bg-white')>
    ...
    <div class="pagecontainer">
        ...
        <div>
            <a href="/myroom/complete/sell?type={{Request::get('type')}}&search_month={{Request::get('search_month')}}" class="btn-endsb @if(Request::get('search_type') != 'previous') selected @endif">
                <input type="radio" name="list_type" value="2" @if(Request::get('search_type') != 'previous') checked @endif onclick="location.href='/myroom/complete/sell'" @class('d-none')> 최근종료내역
            </a>
            <a href="javascript:void(0)" onclick="location.href='/myroom/complete/sell?search_type=previous&type={{Request::get('type')}}'" class="btn-endsb @if(Request::get('search_type') == 'previous') selected @endif">
                <input type="radio" name="list_type" value="1" @if(Request::get('search_type') == 'previous') checked @endif onclick="location.href='/myroom/complete/sell?search_type=previous'" @class('d-none')> 이전종료내역
            </a>
        </div>
        <div class="navtabs__react p-left-0 p-right-0">
            <form id="frmSearch" action="" method="get">
                <input type="hidden" name="type" value="{{Request::get("type")}}" />
                @csrf
                <ul class="g_sideway">
                    <li class="type_area  @if(Request::get('type') != 'buy') f_blue3 border-blue3" @endif">
                        <a href="?type=sell&search_type={{Request::get('search_type')}}&search_month={{Request::get('search_month')}}"><span>팝니다 판매한 내역</span></a>
                    </li>
                    <li @if(Request::get('type') == 'buy') class="f_blue3 border-blue3" @endif>
                        <a href="?type=buy&search_type={{Request::get('search_type')}}&search_month={{Request::get('search_month')}}"><span>삽니다 판매한 내역</span></a>
                    </li>
                    <li>
                    ...
                    <li>
                        <input type="submit" value="검색" class="btn_sumit_complete">
                    </li>
                </ul>
            </form>
        </div>
